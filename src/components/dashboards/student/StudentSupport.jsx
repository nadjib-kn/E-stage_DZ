// src/components/dashboards/student/StudentSupport.jsx
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        setStatus({ type: 'success', text: response.data.message || 'Message sent! Our support team will contact you shortly.' });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 max-w-4xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-800 p-6 md:p-8 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col md:flex-row gap-4 items-start md:items-center justify-between transition-colors duration-300">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Student Support</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Need assistance? Send us a message and our team will get back to you.</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input 
              type="text" 
              required
              className="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-2xl focus:bg-white dark:focus:bg-slate-800 focus:ring-4 focus:ring-blue-500/10 focus:border-[#2563EB] outline-none transition-all text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500"
          ...
          <div>
            ...
            <textarea 
              required
              rows="6"
              className="w-full px-5 py-3.5 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-2xl focus:bg-white dark:focus:bg-slate-800 focus:ring-4 focus:ring-blue-500/10 focus:border-[#2563EB] outline-none transition-all resize-none text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500"
