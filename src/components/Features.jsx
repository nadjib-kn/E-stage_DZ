...
/* Watches <html class="dark"> so the gradient updates on theme toggle */
function useDark() {
...
const Features = () => {
  ...
  const featuresData = [
    {
      ...
      description:
        "Work with top-tier Algerian and international firms. Every company is vetted for quality.",
      iconClasses:
        "text-blue-600 group-hover:bg-blue-600 group-hover:text-white dark:text-blue-400 dark:group-hover:text-white",
      icon: (
        <svg
      ...
      watermarkClasses:
        "-bottom-3 -right-3 w-24 sm:w-28 text-slate-900 dark:text-white",
      watermark: (
        <svg
          viewBox="0 0 76 75"
          fill="none"
        ...
        >
          <g opacity="0.03">
            <path
              d="M0 90V20H20V0H70V40H90V90H50V70H40V90H0ZM10 80H20V70H10V80ZM10 60H20V50H10V60ZM10 40H20V30H10V40ZM30 60H40V50H30V60ZM30 40H40V30H30V40ZM30 20H40V10H30V20ZM50 60H60V50H50V60ZM50 40H60V30H50V40ZM50 20H60V10H50V20ZM70 80H80V70H70V80ZM70 60H80V50H70V60Z"
              fill="currentColor"
            />
          </g>
        </svg>
    ...
    },
    {
      ...
      description:
        "Discover internships perfectly suited to your field of study and technical expertise.",
      iconClasses:
        "text-emerald-500 group-hover:bg-emerald-500 group-hover:text-white dark:text-emerald-400 dark:group-hover:text-white",
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="w-6 h-6"
      ...
      watermarkClasses:
        "-bottom-5 -right-5 sm:-bottom-6 sm:-right-6 w-[120px] sm:w-[140px] text-slate-900 dark:text-white",
      watermark: (
        <svg
          viewBox="0 0 106 100"
          fill="none"
    ...
    },
    {
      ...
      description:
        "Apply with a single click. Track your application status in real-time through your dashboard.",
      iconClasses:
        "text-blue-600 group-hover:bg-blue-600 group-hover:text-white dark:text-blue-400 dark:group-hover:text-white",
      icon: (
        <svg
      ...
      watermarkClasses:
        "-bottom-3 -right-3 sm:-bottom-4 sm:-right-4 w-[110px] sm:w-[130px] text-slate-900 dark:text-white",
      watermark: (
        <svg
          viewBox="0 0 81 65"
          fill="none"
        ...
        >
          <g opacity="0.03">
            <path
              d="M0 80V0L95 40L0 80ZM10 65L69.25 40L10 15V32.5L40 40L10 47.5V65ZM10 65V40V15V32.5V47.5V65Z"
              fill="currentColor"
            />
          </g>
        </svg>
  ...
  // Light mode: transparent -> slate-50/50 -> transparent (maintaining soft edges with updated color)
  // Dark mode: slate-900 
  const sectionBg = isDark
    ? "linear-gradient(to bottom, #0f172a 0%, #0f172a 15%, #0f172a 85%, #0f172a 100%)"
    : "linear-gradient(to bottom, transparent 0%, rgba(248, 250, 252, 0.5) 12%, rgba(248, 250, 252, 0.5) 88%, transparent 100%)";

  return (
    <section 
      className="relative w-full py-12 sm:py-16 md:py-20 lg:py-24 transition-colors duration-500 ease-in-out"
      style={{ background: sectionBg }}
    >
      {/* Subtle Background Glows */}
      <div className="absolute top-0 left-1/4 w-96 h-96 bg-blue-400/10 dark:bg-blue-500/5 rounded-full blur-[100px] pointer-events-none"></div>
      <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-teal-400/10 dark:bg-teal-500/5 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 lg:gap-8 xl:gap-10">
          {featuresData.map((feature) => (
            <div
              key={feature.id}
              className="relative bg-white/70 dark:bg-slate-800/40 backdrop-blur-xl rounded-[1.5rem] sm:rounded-[2rem] border border-white dark:border-slate-700/50 p-6 sm:p-8 md:p-10 flex flex-col items-start h-full shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] hover:shadow-[0_20px_40px_rgba(37,99,235,0.1)] dark:hover:shadow-[0_20px_40px_rgba(37,99,235,0.15)] hover:border-blue-500/30 dark:hover:border-blue-400/30 hover:-translate-y-2 transition-all duration-500 ease-out overflow-hidden group cursor-pointer"
            >
              {/* Icon Container */}
              <div
                className={`w-12 h-12 sm:w-14 sm:h-14 bg-white dark:bg-slate-800 rounded-xl sm:rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center justify-center mb-5 sm:mb-8 z-10 transition-all duration-500 ease-in-out ${feature.iconClasses}`}
              >
                {feature.icon}
              </div>

              {/* Text Content */}
              <h3 className="text-[19px] sm:text-xl md:text-[22px] font-bold text-slate-900 dark:text-white tracking-tight mb-2 sm:mb-3 z-10 transition-colors duration-500 ease-in-out">
                {feature.title}
              </h3>
              <p className="text-[14px] sm:text-[15px] text-slate-500 dark:text-slate-400 leading-relaxed z-10 transition-colors duration-500 ease-in-out">
                {feature.description}
              </p>

              {/* Watermark Base */}
              <div
                className={`absolute pointer-events-none transition-opacity duration-500 ease-in-out group-hover:opacity-70 ${feature.watermarkClasses}`}
              >
                {feature.watermark}
              </div>
            </div>
